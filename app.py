...
def workmaster_daily():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('workmaster_daily'))
    recent_records = daily_records[::-1]
    return render_template('workmaster_daily.html', page_title='歩掛マスター：日々の作業記録', current_app='workmaster_daily', basic_data=basic_data, detail_data=detail_data, recent_records=recent_records)
...
def workmaster_export_excel():
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        basic_df.to_excel(writer, sheet_name='基本情報', index=False)
        detail_df = pd.DataFrame([detail_data])
        detail_df.to_excel(writer, sheet_name='詳細情報', index=False)
    output.seek(0)
    return render_template('workmaster_daily.html', page_title='歩掛マスター：日々の作業記録', current_app='workmaster_daily', basic_data=basic_data, detail_data=detail_data, recent_records=daily_records[::-1], export_message="✅ Excelデータのエクスポート処理が完了しました（ダミー）。")
